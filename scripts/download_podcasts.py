...
def main():
    ...
    ap.add_argument("--once", action="store_true", help="下完一轮就退出（默认下完后休眠待命）")
    ap.add_argument("--idle-sleep", type=int, default=3600, help="下完后每隔多久重扫 feed（默认 1h，礼貌）")
    args = ap.parse_args()
    series = args.series or ORDER
    # 默认循环：下完全部后长眠待命，只每小时礼貌地重扫一次 feed 接新集；
    # 不再退出 → 看门狗不会每 2 分钟把它重拉、空敲 Simplecast。
    while True:
        run(series, args.workers)
        print("==================== 本轮下载完成 ====================", flush=True)
        if args.once:
            break
